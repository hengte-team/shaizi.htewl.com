...
function erweima($uid,$domain){
	    include 'Public/phpqrcode/phpqrcode.php';    
		$value = 'http://'.$domain.'/index.php?mid='.$uid; //二维码内容   
...
function expense($user_info,$money,$type){
    ...
    if($parent_info){
        ...
        $save = array('expense'=>$parent_info['expense']+$expense['money']);
   ...
   //2
    ...
    if($parent_info){
        ...
        M('user')->where(array('id'=>$parent_info['id']))->save(array('expense'=>$parent_info['expense']+$expense['money']));
        $info = M('expense')->add($expense);
        if($info){
            $msg = "你的下级 ".$user_info['nickname']." 成功下单 恭喜您获到佣金奖".$expense['money']."元！\n购买时间:".date('Y-m-d H:i:s');
            sendwen($parent_info['openid'],$msg);
...
function create_qrcode($user_info,$return_false = false){
	...
	$domain=$GLOBALS['_CFG']['site']['tgurl'];
	$path_info['qrcode'] = erweima($uid,$domain);


	// 合成


	$im_dst = imagecreatefromjpeg("./Public/images/768002953599104146.jpg");
